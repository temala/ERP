...
export class InvoiceDetailsComponent implements OnInit , OnChanges{
  ...
  public get totalHT(): number {
    return this.invoiceInfo.invoiceLines.map(l => l.product.priceHT * l.quantity).reduce((x, y) => x + y, 0);
  }

  public get tva(): number {
    return this.invoiceInfo.invoiceLines.map(l => (l.product.priceHT* l.quantity * l.product.tva) / 100).reduce((x, y) => x + y, 0);
  }
